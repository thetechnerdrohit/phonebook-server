...
//   await Area.insertMany(users.map((user) => ({ ...user, password: hashedPassword })));
// };

// describe('User routes', () => {
//   describe('POST /v1/users', () => {
//     let newUser: NewCreatedArea;
...
//       const dbUser = await Area.findById(res.body.id);
...
//   describe('GET /v1/areas/:userId', () => {
//     test('should return 200 and the user object if data is ok', async () => {
//       await insertUsers([userOne]);

//       const res = await request(app)
//         .get(`/v1/areas/${userOne._id}`)
//         .set('Authorization', `Bearer ${userOneAccessToken}`)
//         .send()
//         .expect(httpStatus.OK);
...
//       const dbUser = await Area.findById(userOne._id);
